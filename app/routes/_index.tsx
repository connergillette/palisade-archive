...
export const loader: LoaderFunction = async ({ request }: LoaderArgs) => {
  ...
  const { data: campaigns } = await supabase.from('campaigns').select().order('order')
      ...
      <div className="flex flex-col gap-2 py-10">
        {
          campaigns.map(campaign => (
            <div className="flex gap-20 w-5/6 mx-auto" key={`campaign-${campaign.id}`}>
              <h2 className="text-2xl font-bold w-full">{campaign.name}</h2>
